...
class SendSubscriptionExpiryNotifications extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($expired as $sub) {
            ...
            if ($sub->business) {
                $recipient = $sub->business->owner ?: $sub->business->users()->first();

                if ($recipient) {
                    try {
                        $recipient->notify(new SubscriptionExpired($sub));
                        $this->info("  ✅ Sent expired email to {$recipient->email}");
                    } catch (\Throwable $e) {
    ...
    private function notifyExpiry(Subscription $sub, int $daysRemaining): void
    {
        ...
        try {
            $this->notificationService->notifySubscriptionExpiry($sub, $daysRemaining);
            $this->info("  ✅ Sent expiry email ({$daysRemaining} days) for subscription {$sub->id}");
        } catch (\Throwable $e) {
            $this->error("  ❌ Failed to send expiry notification: {$e->getMessage()}");
        }

        // Also send direct SubscriptionExpiring email to business owner
        if ($sub->business) {
            $recipient = $sub->business->owner ?: $sub->business->users()->first();

            if ($recipient) {
                try {
                    $recipient->notify(new SubscriptionExpiring($sub));
                    $this->info("  ✅ Sent direct expiring email to {$recipient->email}");
                } catch (\Throwable $e) {
                    $this->error("  ❌ Failed to send direct email: {$e->getMessage()}");
                }
            }
        }

        // Keep broadcast for real-time notifications
        $admins = User::where('is_super_admin', true)->get();
        foreach ($admins as $admin) {
            broadcast(new GeneralNotification(
                $admin->id,
