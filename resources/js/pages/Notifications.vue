                    ...
                    >
                        <div class="flex items-start gap-4 p-6">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <!-- Billing Cycle Badge for Subscription Notifications -->
                                <div v-if="notification.type.includes('subscription') && notification.data?.billing_cycle" class="mb-3">
                                    <Badge :class="notification.data.billing_cycle === 'yearly' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-blue-100 text-blue-700 border-blue-200'" class="text-xs font-black uppercase tracking-widest px-3 py-1.5">
                                        {{ notification.data.billing_cycle === 'yearly' ? '📅 Annual Subscription' : '📆 Monthly Subscription' }}
                                    </Badge>
                                </div>
                                ...
                                <div v-if="notification.data && Object.keys(notification.data).length > 0" class="mb-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                                    <div class="text-xs text-slate-600 space-y-1">
                                        <div v-for="(value, key) in notification.data" :key="key" class="flex justify-between">
                                            ...
                                            <span :class="String(key) === 'billing_cycle' ? 'font-bold text-blue-600 uppercase' : 'text-slate-700'" class="break-all">{{ value }}</span>
                                        </div>
                                    </div>
                                </div>
