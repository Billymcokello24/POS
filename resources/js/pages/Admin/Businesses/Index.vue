...
const deleteBusiness = async (id: number, name: string) => {
    if (!confirm(`⚠️ DANGER: Permanently delete "${name}"?\n\nThis will delete:\n• All users\n• All products\n• All sales\n• All data\n\nThis action CANNOT be undone!`)) return
    if (!confirm(`Type the business name to confirm: "${name}"`)) return
    
    router.delete(`/admin/businesses/${id}`, {
        preserveScroll: true,
        onSuccess: () => {
            // Force a full page reload to ensure the business is removed from the list
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-6 border-b border-slate-100">
                    <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="ml-auto flex gap-2">
                          ...
                          <Button class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold" @click="bulkToggleStatus" :disabled="selectedCount===0">Toggle Status</Button>
                          <Button variant="ghost" @click="bulkResetPasswords" :disabled="selectedCount===0" class="text-slate-600 font-medium">Reset Passwords</Button>
                        </div>
                    </div>
                </div>
                ...
                <Table>
                    <TableHeader class="bg-slate-50/50">
                        <TableRow>
                            <TableHead class="w-12 px-4 shadow-none">
                              <input type="checkbox" v-model="allSelected" class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                            </TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="business in props.businesses.data" :key="business.id" class="hover:bg-slate-50/50 transition-colors group">
                            <TableCell class="py-4 px-4">
                                <input type="checkbox" :checked="isSelected(business.id)" @change="toggleSelect(business.id)" class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                            </TableCell>
                            ...
                            <TableCell class="text-right px-6">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" class="w-56 p-2 rounded-2xl shadow-2xl border-slate-200 overflow-hidden">
                                        ...
                                        <DropdownMenuItem @click="deleteBusiness(business.id, business.name)" class="cursor-pointer rounded-xl h-10 hover:bg-red-50 mt-1">
                                            <div class="flex items-center w-full text-red-600">
                                                ...
                                                <span class="font-black text-[11px] uppercase tracking-tighter">Destroy Entity</span>
                                            </div>
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                ...
                <div class="p-6 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
                  <div class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Page Results {{ paginationFrom }}-{{ paginationTo }} of {{ paginationTotal }}</div>
                  <div class="flex items-center gap-2">
                    <Button variant="outline" :disabled="!prevLink" @click="router.get(prevLink)" class="font-bold h-9 px-4 rounded-lg bg-white border-slate-200 text-slate-600 shadow-sm">Prev</Button>
                    <Button variant="outline" :disabled="!nextLink" @click="router.get(nextLink)" class="font-bold h-9 px-4 rounded-lg bg-white border-slate-200 text-slate-600 shadow-sm">Next</Button>
                  </div>
                </div>
